...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg) {
    ...
    radius_filter.setInputCloud(cloud_in);
    radius_filter.setRadiusSearch(0.05);  // Adjust the radius according to your needs
    radius_filter.setMinNeighborsInRadius(5);  // Adjust the minimum number of neighbors according to your needs
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "filter_radius");
    ros::NodeHandle nh;

    filtered_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("filtered_radius_removal", 1);
